process GEN_TARGET_INFO_FROM_GENOMES_NANOPORE {
    ...
    echo -e "target\tforward\treverse" > inner_primers.tsv && for x in `/bin/ls genome_extraction/forSeekDeep/refSeqs/trimmed_toFront25_*.fasta | egrep -v revComp`; do echo -e \$(echo \$(basename \${x%%.fasta}) | sed 's/trimmed_toFront25_//g')"\t"\$(elucidator printSeqs --fasta \${x} | sort | uniq | tr '\n' ',' | sed 's/,\$//g')"\t"\$(elucidator printSeqs --fasta \$(echo \${x} | sed 's/trimmed_toFront25_/trimmed_toFront25_revComp_/g') | sort | uniq | tr '\n' ',' | sed 's/,\$//g') >> inner_primers.tsv; done;
    """
}
